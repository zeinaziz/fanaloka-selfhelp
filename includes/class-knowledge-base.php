<?php
/**
 * KnowledgeBase - editable library of self-help articles.
 *
 ...
 * Detector finds a matching plugin/theme), steps (plain-language guide),
 * source ('builtin' starter content, 'auto' generated by SiteIndexer, or
 * 'manual' written by an admin), and edited (true once an admin has saved
 * changes to a builtin/auto article — protects it from being overwritten
 * or removed the next time SiteIndexer runs).
 *
...
class KnowledgeBase {
	...
	public static function articles(): array {
		...
		$list   = is_array( $stored ) ? $stored : self::seeded_defaults();

		return array_map( array( __CLASS__, 'normalize' ), $list );
	}

	/**
	 * Built-in defaults tagged as 'source' => 'builtin'.
	 *
	 * @return array<int,array<string,mixed>>
	 */
	private static function seeded_defaults(): array {
		return array_map(
	...
	/**
	 * Fill in source/edited defaults for articles saved before those fields
	 * existed, so older stored data doesn't break SiteIndexer's assumptions.
	 *
	...
	public static function maybe_seed_defaults(): void {
		if ( false === get_option( self::OPTION_KEY, false ) ) {
			update_option( self::OPTION_KEY, self::seeded_defaults() );
		}
	}
